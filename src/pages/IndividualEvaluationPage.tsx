...
export const IndividualEvaluationPage: React.FC = () => {
  ...
  const [adjustmentScore, setAdjustmentScore] = useState<number>(existingEval?.adjustmentScore || 0);
  const [adjustmentReason, setAdjustmentReason] = useState<string>(existingEval?.adjustmentReason || '');
  ...
  // Calculate live total score (Barema + Adjustment clamped to 0..20)
  const liveBaremaScore = settings.baremaCriteria.reduce((acc, crit) => {
    const scoreVal = criterionScores[crit.id] ?? 0;
    return acc + Math.max(0, Math.min(scoreVal, crit.maxScore));
  }, 0);

  const liveTotalScore = Math.max(0, Math.min(20.0, liveBaremaScore + (Number(adjustmentScore) || 0)));
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#C20054] text-white font-bold text-lg shadow-sm">
              {student.name.substring(0, 2).toUpperCase()}
            </div>
            <div>
              <h2 className="text-lg font-bold text-[#C20054] dark:text-blue-400">
                {student.name}
              </h2>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900 space-y-5">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-slate-800">
            ...
            <div className="rounded-xl bg-indigo-900 px-4 py-2 text-white text-right shadow-xs">
              ...
              <span className="text-lg font-black">{liveBaremaScore.toFixed(1)} / 20.0</span>
            </div>
          </div>
      ...
      {/* Manual Teacher Adjustment Card */}
      {attendance === 'Presente' && (
        <div className="rounded-2xl border border-indigo-200 bg-white p-5 shadow-xs dark:border-indigo-900/60 dark:bg-slate-900 space-y-4">
          <div className="flex items-center justify-between border-b border-indigo-100 pb-3 dark:border-indigo-900/40">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  Permite somar ou subtrair pontos para ajustar a nota final do aluno
                </p>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-5 sm:grid-cols-9 gap-1.5">
              {[-2.0, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 2.0].map((preset) => {
                ...
                return (
                  <button
                    key={preset}
                    type="button"
                    onClick={() => setAdjustmentScore(preset)}
          ...
          {/* Custom fine-step stepper & Justification */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <div className="space-y-1.5">
              <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Valor Fino do Ajuste:
              </label>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => setAdjustmentScore((prev) => Math.round((prev - 0.5) * 10) / 10)}
                ...
                <input
                  ...
                  onChange={(e) => setAdjustmentScore(parseFloat(e.target.value) || 0)}
                  className={`h-9 w-20 rounded-xl border text-center text-sm font-bold font-mono ${
                    adjustmentScore > 0
                ...
                <button
                  type="button"
                  onClick={() => setAdjustmentScore(0)}
                  className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-2.5 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 ml-1"
                >
                  Zerar
                </button>
              </div>
            ...
            <div className="space-y-1.5">
              <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Justificativa Pedagógica (Opcional):
              </label>
              <input
                type="text"
                placeholder="Ex.: Síntese clínica de excelência..."
                value={adjustmentReason}
                onChange={(e) => setAdjustmentReason(e.target.value)}
                className="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-3.5 py-2 text-xs text-slate-800 dark:text-slate-100 placeholder:text-slate-400"
              />
            </div>
          </div>
        </div>
